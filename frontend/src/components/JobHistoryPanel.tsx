...
export const JobHistoryPanel: React.FC<JobHistoryPanelProps> = ({ refreshTrigger, layout = "classic" }) => {
  ...
  const getJobTypeLabel = (type?: string) => {
    switch (type) {
      case "clone_voice":
        return { label: "Clone Voice", color: "text-indigo-400 border-indigo-500/20 bg-indigo-500/5" };
      case "auto_voice":
        return { label: "Auto Voice", color: "text-sky-400 border-sky-500/20 bg-sky-500/5" };
      case "voice_design":
        return { label: "Voice Design Direct", color: "text-purple-400 border-purple-500/20 bg-purple-500/5" };
      case "voice_design_preview":
        return { label: "Voice Design Preview", color: "text-pink-400 border-pink-500/20 bg-pink-500/5" };
      default:
        return { label: "TTS Job", color: "text-slate-400 border-slate-500/20 bg-slate-500/5" };
    }
  };
                  ...
                  <button
                    ...
                    className={`py-2 px-1 text-center font-bold text-xs rounded-lg transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                      saveVoiceIsPublic
                        ...
                        : "text-slate-450 hover:text-slate-355"
                    }`}
                  >
