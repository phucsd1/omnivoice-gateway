        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredVoices.map((voice) => {
            ...
            return (
              <div
                key={voice.id}
                className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 flex flex-col justify-between gap-4 hover:border-slate-700/80 transition-all shadow-md group relative"
              >
                {/* Delete Confirm Overlay */}
                {deleteConfirmId === voice.id && (
                  <div className="absolute inset-0 bg-slate-950/95 rounded-2xl p-4 flex flex-col justify-center items-center gap-3 z-10">
                    <span className="text-xs font-bold text-center text-slate-200">
                      Bạn có chắc chắn muốn xóa giọng mẫu "{voice.name || voice.id}"?
                    </span>
                    <div className="flex gap-2">
                      <button
                        onClick={() => setDeleteConfirmId(null)}
                        className="px-3 py-1.5 bg-slate-800 text-xs font-semibold hover:bg-slate-700 text-slate-300 rounded-lg cursor-pointer transition-all"
                      >
                        Hủy
                      </button>
                      <button
                        onClick={() => handleDelete(voice.id)}
                        className="px-3 py-1.5 bg-rose-600 text-xs font-bold hover:bg-rose-500 text-white rounded-lg cursor-pointer transition-all"
                      >
                        Đồng ý Xóa
                      </button>
                    </div>
                ...
                <div className="flex flex-col gap-2.5">
                  <div className="flex justify-between items-start gap-2">
                    <span className="font-extrabold text-sm text-white truncate group-hover:text-indigo-300 transition-colors">
                      {voice.name || "Giọng không tên"}
                    </span>
                    <div className="flex items-center gap-1.5 shrink-0">
                      {/* Visibility Badge */}
                      {voice.is_public ? (
                        <span className="bg-emerald-500/10 border border-emerald-500/25 text-[9px] font-extrabold text-emerald-450 px-1.5 py-0.5 rounded-full flex items-center gap-1">
                          <Globe className="w-2.5 h-2.5" />
                          <span>Public</span>
                        </span>
                      ) : (
                        <span className="bg-indigo-500/10 border border-indigo-500/25 text-[9px] font-extrabold text-indigo-400 px-1.5 py-0.5 rounded-full flex items-center gap-1">
                      ...
                      {/* Source Type Badge */}
                      <span className="bg-slate-800 border border-slate-700 text-[9px] font-bold text-slate-400 px-1.5 py-0.5 rounded-full capitalize">
                  ...
                  <div className="flex items-center gap-1 text-[10px] text-slate-450 font-mono">
                    ...
                    <span className="truncate max-w-[140px] select-all bg-slate-950 border border-slate-850 px-1.5 py-0.5 rounded text-slate-350">
                      {voice.id}
                    </span>
                    <button
                  ...
                  {/* Ref text section */}
                  {voice.ref_text && (
                    <div className="bg-slate-950/60 border border-slate-850 rounded-xl p-3 max-h-[80px] overflow-y-auto text-xs text-slate-300 leading-normal scrollbar-thin">
                      <p className="font-semibold text-[10px] text-slate-500 uppercase tracking-wider mb-1">Văn bản tham khảo</p>
                      {voice.ref_text}
                    </div>
                  )}
                ...
                <div className="flex flex-col gap-2 pt-2 border-t border-slate-850">
                  ...
                  <div className="flex gap-2 mt-1">
                    <button
                      ...
                      className="flex-grow py-2 px-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-md shadow-indigo-600/10 hover:translate-y-[-1px]"
                    >
                    ...
                    {!voice.is_public && (
                      <button
                        ...
                        className="p-2 border border-slate-800 hover:border-rose-500/35 hover:bg-rose-500/5 text-slate-400 hover:text-rose-400 rounded-xl transition-all cursor-pointer"
